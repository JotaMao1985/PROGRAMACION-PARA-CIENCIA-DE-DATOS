...
try:
    ...
    if COL_DEPTO in df.columns:
        unique_deptos = df[COL_DEPTO].unique()
        
        df_normalized = pd.DataFrame({'Original_CSV': unique_deptos})
        df_normalized['Normalized_Key_CSV'] = df_normalized['Original_CSV'].apply(normalize_depto_names)
        
        print("--- Diagnóstico de Claves de Unión del Archivo CSV (Versión 3) ---")
        print("Se aplicó la función `normalize_depto_names` re-ordenada.")
        
        # Mostrar la tabla de normalización completa
        print(df_normalized.to_markdown(index=False))
        
        # Mostrar específicamente el caso de Bogotá
        bogota_rows = df_normalized[df_normalized['Original_CSV'].str.contains("BOGOTA", case=False, na=False)]
        print("\n--- Enfoque en 'BOGOTA' ---")
        if not bogota_rows.empty:
            print(bogota_rows.to_markdown(index=False))
        else:
            print("No se encontró 'BOGOTA' en la columna de departamentos del CSV.")
            
    else:
        print(f"Error: La columna '{COL_DEPTO}' no se encontró en el CSV.")
        print("Columnas disponibles:")
        print(df.columns.tolist())

except FileNotFoundError:
    print("Error: No se encontró el archivo 'MEN_SNIES_test.csv'.")
...
with tab2:
    ...
    with st.spinner("Cargando mapa..."):
        # Creamos el DF para el mapa, ignorando el filtro de depto
...
with tab4:
    ...
    with st.spinner("Preparando datos..."):
        # Streamlit: Primero preparamos los datos para la descarga
        
        @st.cache_data # Cachear la conversión a CSV
        def convert_df_to_csv(df):
            # fileEncoding = "UTF-8" en R con BOM (común para Excel) es "utf-8-sig"
